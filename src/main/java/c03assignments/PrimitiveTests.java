...
//		int x = 3957.229; // illegal: explicit cast needed to convert double to int.
		...
		long l3 = 56L;
		byte b3 = (byte)l3;
		System.out.println("The byte is " + b3);// um valor esquisito mas sem compile error
		
		byte b4 = 3;
		b4 += 7;// +=, -=, *=, and /= will all put in an implicit cast
		b4= (byte) (b + 7);		// Won't compile without the cast, since b + 7 results in an int

		Integer oi = Integer.valueOf(44);
		Integer oi2 = Integer.parseInt("44"); // autoboxing
//		Double oi2 = Integer.parseInt("44"); // NÃO!
		String s = "a";
		s += b4;
